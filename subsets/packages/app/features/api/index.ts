...
export const fetchStatus = async (): Promise<Status> => {
    ...
    var nextGuess = (data.nextGuess) ? {
      characters: data.nextGuess.characters.map((clue: any): Clue => ({
        letter: clue.letter || '',
        clueType: (typeof clue.type === 'number' && ClueType[clue.type] !== undefined)
          ? clue.type as ClueType : ClueType.Empty,
      })),
    ...
    return {
        ...
        guesses: data.guesses.map((guess: any): Guess => ({
            characters: guess.characters.map((clue: any): Clue => ({
                letter: clue.letter || '',
                clueType: (typeof clue.type === 'number' && ClueType[clue.type] !== undefined)
                  ? clue.type as ClueType : ClueType.Empty,
            })),
